...
<div class="d-flex px-4 justify-content-between pt-4 align-items-center border-bottom product-bar">
    <p class="products-title px-4">Add new product</p>
</div>
<form method="POST" action="{{ route('admin.add') }}" enctype="multipart/form-data">
    ...
    <div class="row py-4 px-5">
        <div class="col-12 col-sm-6 px-5 mb-3">
            ...
            <input type="text" name="product_name" id="productName" class="input-box form-control" required>
        </div>
        <div class="col-12 col-sm-6 px-5 mb-3">
            <label for="productPrice" class="form-label">Product Price</label>
            <input type="number" name="product_price" id="productPrice" min="0" step="0.01" class="input-box form-control" required>
        </div>
        <div class="col-12 col-sm-6 px-5 mb-3">
            <label for="productColor" class="form-label">Product Color</label>
            <input type="text" name="product_color" id="productColor" class="input-box form-control" required>
        </div>
        <div class="col-12 col-sm-6 px-5 mb-3">
        ...
        <div class="col-12 px-5 mb-3">
            ...
            <input type="text" name="brief_description" id="briefDescription" class="input-box form-control" required>
        </div>
        <div class="col-12 px-5 mb-3">
            <label for="detailedDescription" class="form-label">Detailed description</label>
            <textarea type="text" name="detailed_description" id="detailedDescription" class="input-box form-control" style="height: 100px;"></textarea>
        </div>
        <div class="col-12 px-5 mb-3">
            <label for="featuresSpecifications" class="form-label">Features & Specifications</label>
            <textarea type="text" name="features" id="featuresSpecifications" class="input-box form-control" style="height: 100px;"></textarea>
        </div>
        <div class="col-12 px-5 mb-3">
        ...
        <div class="col-12 col-sm-6 px-5 mb-3">
            ...
            <div class="d-flex flex-wrap gap-3">
                @foreach($mainCategories as $category)
                    <input type="radio" name="category" data-name="{{ $category->name }}" class="btn-check parent-category" id="category-{{ $category->id }}" value="{{ $category->id }}" autocomplete="off">
            ...
            <div class="d-flex flex-wrap gap-3">
                ...
                @foreach($tags['audience'] ?? [] as $tag)
                    <input type="checkbox" name="tags[]" class="btn-check audience-tag" data-name="{{ $tag->name }}" id="tag-{{ $tag->id }}" value="{{ $tag->id }}" autocomplete="off">
                    <label for="tag-{{ $tag->id }}" class="btn btn-outline-dark">{{ $tag->name }}</label>
                @endforeach
